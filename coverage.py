...
def parse_coverage(file_name):
    xmldoc = minidom.parse(file_name)
    root = xmldoc.getElementsByTagName("scoverage")
    return root[0].attributes['statement-rate'].value
...
def parse_args(argv=None):
    ...
    parser.add_argument('-c', dest='coverage', default=None,
                        help='The coverage value for the file')
    parser.add_argument('-cf', dest='coverage_file', default='scoverage.xml',
                        help='Path to the scala coverage file')
...
def main(argv=None):
    ...
    # Generate badge
    total = args.coverage or parse_coverage(args.coverage_file)
